...
/* Constructor */
template<class FloatT>
DEstimator<FloatT>::DEstimator(const MatT &Obs_, const VecT &T_, const MatT &V_, FloatT exposureT_)
    : Ndim(static_cast<int>(Obs_.n_cols)), N(static_cast<int>(Obs_.n_rows)),
      Obs(Obs_), T(T_), V(V_), exposureT(exposureT_)
{
    if(Obs.n_rows!=T.n_elem) throw std::length_error("Obs and T length missmatch.");
    if(Obs.n_rows!=V.n_rows) throw std::length_error("Obs and V length missmatch.");
    if(Obs.n_cols!=V.n_cols) throw std::length_error("Obs and V dim missmatch.");
    dT = vec_delta(T);
    vM.set_size(N);
    vD.set_size(N-1);
}
...
FloatT DEstimator<FloatT>::LLH(FloatT D)
{
    ...
    for(int dim=0; dim<Ndim; dim++){
        ...
        computeVariance(D,dT,V.col(dim),exposureT,min_variance,vD,vM);
...
void DEstimator<FloatT>::LLH(const VecT &D, VecT &LLH)
{
    ...
    #pragma omp parallel
    {
        ...
        for(int k=0; k<ND; k++){
            for(int dim=0; dim<Ndim; dim++) {
                FloatT *pObs = Obs.memptr()+dim*N; //Select out the "dim" column from the matrix of observations
                computeVariance(D(k),dT,V.col(dim),exposureT,min_variance,local_vD,local_vM);
...
void DEstimator<FloatT>::LLH_recursive1D(const VecT &D, const VecT &Obs, 
                        const VecT &T, const VecT &V, FloatT exposureT,  VecT &LLH)
{
    int N = static_cast<int>(Obs.n_elem);
    int ND = static_cast<int>(D.n_elem);
    checkLLHargs(D,Obs,T,V,LLH);
    VecT dT = vec_delta(T);
    #pragma omp parallel
    {
        VecT vD(N-1);
        ...
        #pragma omp for
        for(int k=0;k<ND;k++){
            computeVariance(D(k),dT,V,exposureT,min_variance,vD,vM);
...
template<class FloatT>
void DEstimator<FloatT>::LLH_laplace1D( const VecT &D, const VecT &Obs, const VecT &T, const VecT &V, FloatT exposureT, VecT &LLH)
{
    int N = static_cast<int>(Obs.n_elem);
    int ND = static_cast<int>(D.n_elem);
    checkLLHargs(D,Obs,T,V,LLH);
    ...
    #pragma omp parallel
    {
        ...
        #pragma omp for
        for(int k=0;k<ND;k++){
            computeVariance(D(k),dT,V,exposureT,min_laplace_variance,vD,vM);
            int vMsign = 1;
...
template<class FloatT>
void DEstimator<FloatT>::LLH_markov1D( const VecT &D, const VecT &Obs, const VecT &T, const VecT &V, FloatT exposureT, VecT &LLH)
{
    int N = static_cast<int>(Obs.n_elem);
    int ND = static_cast<int>(D.n_elem);
    checkLLHargs(D,Obs,T,V,LLH);
...
template<class FloatT>
void DEstimator<FloatT>::computeVariance(FloatT D, const VecT &dT, const VecT &V,
                                         FloatT exposureT ,FloatT min_variance, VecT &vD, VecT &vM)
{
    int N = static_cast<int>(V.n_elem);
    D=fabs(D);
    vD = 2*D*dT;
    for(int n=0; n<N-1; n++) if(vD(n)<min_variance) vD(n)=min_variance;
    vM = V-2*D*exposureT/6;
    for(int n=0; n<N; n++) {
...
template<class FloatT>
inline void DEstimator<FloatT>::checkLLHargs(const VecT &D, const VecT &Obs, const VecT &T, const VecT &V,  VecT &LLH)
{
    if(D.n_elem<1) throw std::length_error("len(D)<1.");
    if(Obs.n_elem<2) throw std::length_error("len(Obs)<2.");
    if(Obs.n_rows!=T.n_elem) throw std::length_error("Obs and T length missmatch.");
    if(Obs.n_elem!=V.n_elem) throw std::length_error("Obs and V length missmatch.");
