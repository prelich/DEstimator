 ...
 *
 * The class is instantiated with the observations, variance and times for a single observed trajectory.  The trajectory can be
...
public:
    ...
    MatT V;  // size:[N x Ndim] The estimated variance of each observed position in each dimension
     ...
     * @param[in] V - size:[N x Ndim] The estimated variance of each observed position in each dimension
     * @param[in] exposureT - The exposure time over which each observation was collected
     */
    DEstimator(const MatT &Obs, const VecT &T, const MatT &V, FloatT exposureT);
     ...
     * @param[in] V - A vector of estimated variances for the 1D observations
     * @param[in] exposureT - The exposure time over which each observation was collected.
     * @param[out] LLH - The computed log-likelihood for each of the given D values
     */
    static void LLH_recursive1D( const VecT &D, const VecT &Obs, const VecT &T,  const VecT &V, FloatT exposureT, VecT &LLH);
     ...
     * @param[out] LLH - The computed log-likelihood for each of the given D values
     */
    static void LLH_laplace1D( const VecT &D, const VecT &Obs, const VecT &T, const VecT &V, FloatT exposureT, VecT &LLH);
     ...
     * @param[out] LLH - The computed log-likelihood for each of the given D values
     */
    static void LLH_markov1D( const VecT &D, const VecT &Obs, const VecT &T, const VecT &V, FloatT exposureT, VecT &LLH);

private:
    static const FloatT min_variance; //The smallest the a variance is allowed to get to zero in absolute value in the recursive and Markov methods
    static const FloatT min_laplace_variance; //The smallest the a variance is allowed to get to zero in absolute value in the Laplace method
     ...
     * @param[in] V - The estimated observation variances
     * @param[in] exposureT - The exposure time over which each observation was collected.
     ...
     */
    static void computeVariance(FloatT D, const VecT &dT, const VecT &V, FloatT exposureT, FloatT min_variance, VecT &vD, VecT &vM);
     ...
     */
    static void checkLLHargs(const VecT &D, const VecT &Obs, const VecT &dT, const VecT &V, VecT &LLH);
};
